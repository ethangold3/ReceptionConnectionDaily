...
def update_daily_players():
    ...
    for _ in range(max_attempts):
        ...
        strong_nodes = [node for node in G.nodes() if calculate_cumulative_strength(node) >= 7000]
        if not strong_nodes:
        ...
        strong_end_nodes = [node for node in nodes_two_away if calculate_cumulative_strength(node) >= 7000]
